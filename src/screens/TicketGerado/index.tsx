...
export default function TicketGerado(){
   ...
   let valueReal = products.reduce((initial,product)=>initial+ (Number(product.quantidadeUser)*product.productPrice),0).toFixed(2)
    ...
    <View style={style.container}>
        ...
        renderItem={({item})=>(
            ...
            <ItemListAddTicket productName={item.productName} productUni={item.quantidadeUser} productPrice={(item.productPrice * Number(item.quantidadeUser)).toFixed(2)}></ItemListAddTicket>
    ...
    />
        ...
        <View style={style.infoSub}>
            <Text>Quantidade de itens: {products.reduce((initial,product)=>initial+ Number(product.quantidadeUser),0)}</Text>
        ...
        <ButtonDefault fuc={()=>{ getControlTicket()}} name="IMPRIMIR"></ButtonDefault>
    </View>
