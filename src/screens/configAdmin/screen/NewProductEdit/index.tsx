...
export default function NewProductEdit(){
        ...
        function updateDB(){
            ...
            Products.update(newProductID,{productPrice:newProductPrice,productUni:newProductUni}).then(()=>{console.log("Criado com sucesso" + {productName})})
    ...
    return(
        <View style={styles.container}>
            ...
            <TextInput style={styles.input} onChangeText={(item)=>{setProductName(item)}} defaultValue={router.params.productName} editable={false} > </TextInput>

            <Text style={styles.text}>{"Valor Atual: "+productPrice}</Text>
            <TextInput style={styles.input} keyboardType="numeric" onChangeText={(item)=>{setNewValue(item)}}></TextInput>

            <Text style={styles.text}>Quantidade atual é:   {productUni}</Text>
            <TextInput style={styles.input} keyboardType="numeric" onChangeText={(item)=>{setNewUni(item)}}></TextInput>
